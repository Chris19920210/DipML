...
def to_unicode(s, ignore_errors=False):
    if is_unicode(s):
        ...
        if not ignore_errors:
            raise UnicodeDecodeError("utf-8", s, 0, 1, "Oops, it's buggy")
...
class SubwordTextEncoder(TextEncoder):
    ...
    def build_to_target_size(cls,
        ...
        Args:
          ...
          lower_bound: An integer; lower bound for the minimum token count.
          upper_bound: An integer; upper bound for the minimum token count.
          max_subtoken_length: Maximum length of a subtoken. If this is not set,
            then the runtime and memory use of creating the vocab is quadratic in
            the length of the longest token. If this is set, then it is instead
            O(max_subtoken_length * length of longest token).
    ...
    def build_from_token_counts(self,
        ...
        # alphabet plus the escaping characters.import numpy as np
