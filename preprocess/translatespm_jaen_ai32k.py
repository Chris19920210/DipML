...
_NC_TRAIN_DATASETS = [[
    "train.tgz", [
        "train/en-ja.ja.tok.train",
        "train/en-ja.en.tok.train"
    ]
]]
...
_NC_TEST_DATASETS = [[
    "test.tgz",
    ["test/en-ja.ja.tok.test",
     "test/en-ja.en.tok.test"]
]]
...
class TranslatespmJaenAi32k(translate.TranslateProblem):
    ...
    def generate_encoded_samples(self, data_dir, tmp_dir, dataset_split):
        ...
        filename_base = "wmt_jaen_%sk_tok_%s" % (self.approx_vocab_size, tag)
    ...
    def feature_encoders(self, data_dir):
        source_vocab_filename = os.path.join(data_dir, self.source_vocab_name+'.model')
        target_vocab_filename = os.path.join(data_dir, self.target_vocab_name+'.model')
        source_token = SpmTextEncoder(source_vocab_filename)
        target_token = SpmTextEncoder(target_vocab_filename)
        return {
            "inputs": source_token,
